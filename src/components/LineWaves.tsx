/**
 * Line Waves — React Bits (https://reactbits.dev) — fond animé à lignes.
 */
...
export default function LineWaves({
  ...
  useEffect(() => {
    ...
    function movePoints(time: number) {
      ...
      lines.forEach((pts) => {
        pts.forEach((p) => {
          const move =
            noise.perlin2((p.x + time * cfg.waveSpeedX) * 0.002, (p.y + time * cfg.waveSpeedY) * 0.0015) * 12
  ...
  return (
    <div
      ref={containerRef}
      className={`line-waves ${className}`.trim()}
      style={{ backgroundColor, ...style }}
      aria-hidden
    >
      <canvas ref={canvasRef} className="line-waves__canvas" />
    </div>
  )
}
