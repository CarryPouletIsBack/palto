...
/** Table palette seule */
export function PaltoPaletteTable({ projectData, isEn, t }: BaseProps) {
  const ds = projectData.designSystem;
  if (!ds) return null;
  const paletteRows = buildPaletteRows(projectData, isEn);

  return (
    <div className="palto-ds-tables palto-ds-tables--palette">
...
export function PaltoTypescaleTable({
  projectData,
  t,
  ...
  const bentoTile = typographyHeadingId === 'palto-bento-typescale';

  return (
    <div className={`palto-ds-tables palto-ds-tables--typescale${bentoTile ? ' palto-ds-tables--typescale-bento' : ''}`}>
